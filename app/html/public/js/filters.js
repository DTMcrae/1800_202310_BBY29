function filterResults(urgencies, categories, allowClosed) {
  ...
  for (var i = 0; i < requestsToFilter.length; i++) {
    ...
    if (
      categories.includes(
        requestsToFilter[i]
          .querySelector(".request-category")
    ...
    if (
      urgencies.includes(
        requestsToFilter[i]
          .querySelector(".request-urgency-text")
          .innerHTML.toLowerCase()
      ) ||
      urgencies.length <= 1
    ) {
    ...
    if (
      requestsToFilter[i].className.split(" ").includes("request-archived") &&
      !allowClosed
    ) {
      showClosed = false;
    }

    if (hasCategory && hasUrgency && showClosed)
...
function hideMenu() {
  ...
  document
    ...
    .setAttribute("onClick", "displayMenu()");
}
